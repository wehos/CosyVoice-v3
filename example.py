...
def cosyvoice3_example():
    ...
    """
    cosyvoice = AutoModel(model_dir='pretrained_models/Fun-CosyVoice3-0.5B', load_trt=True)
    # zero_shot usage
    ...
    for i, j in enumerate(cosyvoice.inference_zero_shot(text_generator(), 'You are a helpful assistant.<|endofprompt|>希望你以后能够做的比我还好呦。',
                                                        './asset/zero_shot_prompt.wav', stream=True)):
        total.append(j['tts_speech'])
    torchaudio.save('hotfix.wav', torch.concat(total, 1), cosyvoice.sample_rate)
